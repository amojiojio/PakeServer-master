package com.pk.server.wechat.api.ctrl;

import com.pk.server.wechat.api.process.*;
import com.pk.server.wechat.api.service.impl.MyServiceImpl;
import com.pk.server.wechat.api.vo.*;
...
import com.pk.server.wechat.core.spring.JsonView;
import com.pk.server.wechat.core.util.DateUtil;
import com.pk.server.wechat.core.util.UploadUtil;
import com.pk.server.wechat.core.util.wx.SignUtil;
...
public class WxApiCtrl {
    ...
    @ResponseBody
    public String publishMenu(HttpServletRequest request, String gid) {
    ...
    public ModelAndView syncAccountFansList() {
        ...
        if (mpAccount != null) {
            ...
            if (flag) {
                return new ModelAndView("redirect:/admin/accountfans/fansPagination.action");
            }
        }
    ...
    public ModelAndView syncMaterials(PageEntity pageEntity) {
        Pagenation pagenation = new Pagenation();
        MpAccount mpAccount = WxMemoryCacheClient.getSingleMpAccount();//获取缓存中的唯一账号
        ModelAndView mv = new ModelAndView("wxcms/materialPagination");
        Integer offset = pageEntity.getStartRow();
        Integer count = pageEntity.getPageSize();
        ...
        if (material != null) {
            ...
            pagenation.setPageCount(material.getTotalCount());
            pagenation.setList(materialList);
        }
        mv.addObject("page", pagenation);
        return mv;
    }
