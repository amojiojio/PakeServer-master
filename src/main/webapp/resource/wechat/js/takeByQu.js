...
$('#E_Con').on('click', function() {
    layer.open({
        ...
        style: 'position:fixed; bottom:0; left:0; width: 100%; height: 50%; padding:10px 0; border:none;overflow:auto'
    });
});
...
$(function () {
    ...
    timePlugByM(
        '.time_plugByM',
        '请选择短信接收时间',
        '.time_plugByM .card_cByM'
    );
    //时间控件脚本
    function timePlugByM(showL, genre, loca) {
        var opt={};
        ...
        $("#appDateTimeByM").val('01').mobiscroll(optDateTime).datetime(optDateTime);
        $("#appDateTimeByM").hide();
        ...
        $(showL).on('click', function () {
            $('#appDateTimeByM').val('01').mobiscroll('show');
            modify();
            return false;
        });

        function modify() {
            for (var i = 0; i < 5; i++) { //隐藏时间控件年月日
            ...
            //--------------------------------------------
            $('.dwwr').css('width', '100%');
            show_list.find('.dw-li.dw-v').eq(0).text('更早之前');
            show_list.find('.dw-li.dw-v').eq(1).text('三天前');
            show_list.find('.dw-li.dw-v').eq(2).text('前天');
            show_list.find('.dw-li.dw-v').eq(3).text('昨天');
            show_list.find('.dw-li.dw-v').eq(4).text('今天');
            if ($('.dwv')) {
            ...
            $('.dwbw.dwb-s .dwb').on('touchstart', function () {
                //获得时间
                var timeArr = [];
                for (var i = 0; i <= 5; i++) { //获取所有列中的时间
                    var timeVal = $('.dwwl.dwrc.dwwl' + i).find('.dw-li.dw-v.dw-sel').text();
                    timeArr.push(timeVal);
                }
                //输出时间
                $(loca).val(timeArr[last_date]);
                $("#messageTime").val(timeArr[0]);
                setTimeout("position_toSelect('PriceToken_1')", 20);
            });
            $('.dwbw.dwb-s').prepend('<div class="liji">今天</div>');
            $('.liji').on('click', function () {
                $('#appDateTimeByM').mobiscroll('hide');
                $(loca).val('今天');
                setTimeout("position_toSelect('PriceToken_1')", 20);
            });
